...
//If the lastCommitIDBuiltFile does not exist or is empty we need to build everything...assume it is first time running
if(!lastCommitIDBuiltFile.exists() || lastCommitIDBuiltFile.text == '')
{
    //previousCommitID = "HEAD~1"
        
     println("** Previous commitID not found.  Assuming this is first time build, so build everything...")   
...
else
{
    ...
    println "previousCommitID="+previousCommitID
        ...
        {
           println "!!!!!!! Commit already built, nothing to do - " + previousCommitID + " = " + commitID
        }    
}
